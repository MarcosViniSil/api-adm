...
async def create_animal(registerAnimal:RegisterAnimal,credentials: HTTPAuthorizationCredentials = Depends(security),animalService: AnimalService = Depends(getAnimalService)):
    return animalService.createAnimal(credentials.credentials,registerAnimal)
...
async def delete_animal(animalId:int,credentials: HTTPAuthorizationCredentials = Depends(security),animalService: AnimalService = Depends(getAnimalService)):
    return animalService.deleteAnimalById(credentials.credentials,animalId)
